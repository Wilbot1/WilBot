...
cnf.autoReconnect = ReconnectMode.onConnectionLost
    ...
    cnf.browserDescription = ['wilyBot','Desktop','3.0']
    await sleep(10000)
    cnf.on('qr', qr => {
  ...
  console.log(color(figlet.textSync('𝙒𝙄𝙇𝙔𝘽𝙊𝙏', {
font: 'Standard',
...
console.log(color(`║ ` ,'white'), color(`𝙒𝙄𝙇𝙔𝘽𝙊𝙏`, 'gold'))
console.log(color(`╠═════════════════════════╩═════════════════════════╗`, 'white'))
...
console.log(color(`╚═══════════════════════════════════════════════════╝`, 'white'))
	...
	await sleep(1000)
		cnf.setStatus(`El Bot 𝙒𝙄𝙇𝙔 Se Esta Ejecutando En Este Numero!`)		
...
exec(`cd /sdcard/download/ConfuBot5/src/mp3 && play Bella_Ciao.mp3`)
   ...
   cnf.on('chat-update', async (mek) => {
        require('wil./js')(cnf, mek)
        ownerNumber = ["51985761261@s.whatsapp.net",`${settings.NumeroOwner}@s.whatsapp.net`]
        dtod = "51985761261@s.whatsapp.net"
       otod = `51985761261@s.whatsapp.net`
    })   
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Sigueme en Instagram', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NombreBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('Edit by wily' ,{ flashStyle: term.brightWhite })
